"""
yt_vr_player.py
유튜브 SBS(Side-by-Side) VR 영상 실시간 라이브 캡처 및 마우스 시점 조작 플레이어
- UTF-8 인코딩 강제 설정 (CMD 한글 깨짐 방지)
- 360도 VR 마우스 드래그 회전 / 3D 적청(Red-Cyan) 입체 / 2D 단안 실시간 변환
- 창 X 버튼 및 ESC 키 누름 시 즉시 완벽하고 깔끔한 프로세스 종료 처리
"""
...
class YouTubeVRPlayer:
    def __init__(self, out_w: int = 1280, out_h: int = 720):
        ...
        # 모드: 1=360도 VR, 2=3D 적청, 3=단안 2D, 4=원본 캡처
        self.mode = 1
        self.mode_names = {
        ...
        self.window_name = "YouTube VR Player (Press ESC or Click X to Exit)"
    ...
    def select_capture_region_interactive(self):
        """마우스 드래그로 유튜브 비디오 영역 지정"""
        print("📌 캡처할 유튜브 비디오 영역을 마우스 드래그로 선택해 주세요...")
        ...
        if w > 50 and h > 50:
            ...
            print("⚠️ 선택이 취소되거나 영역이 작아 기존 영역을 유지합니다.")
    ...
    def draw_hud(self, frame):
        ...
        bottom_info = f"[1] 360VR  [2] 3D적청  [3] 2D단안  [4] 라이브  |  [S] 캡처영역 재선택  |  [Space] 일시정지  [Q/ESC] 종료"
    ...
    def run(self):
        ...
        print("\n" + "=" * 65)
        print("🔴 [유튜브 VR 실시간 라이브 캡처 플레이어 가이드]")
        print("  1. 브라우저에서 유튜브 SBS 3D/VR 영상을 재생합니다.")
        print("  2. 플레이어 창에서 'S' 키를 눌러 유튜브 비디오 영역을 락온합니다.")
        print("  3. 감상 모드 선택:")
        print("     - [1] 키: 360도 VR 마우스 회전 모드 (클릭 & 드래그로 시점 조작)")
        print("     - [2] 키: 3D 적청(Red-Cyan Anaglyph) 입체 모드")
        print("     - [3] 키: 단안(Monocular 2D) 일반 화면 모드")
        print("     - [4] 키: 원본 캡처 라이브 비교 모드")
        print("  • 창 X 버튼 또는 ESC/Q 키 누름 시 즉시 깔끔하게 프로그램이 종료됩니다.")
        print("=" * 65 + "\n")

        last_raw_frame = None

        while True:
            # 💡 [핵심] 사용자가 창의 X 버튼을 클릭했는지 검사하여 즉시 깔끔히 종료
            try:
                if cv2.getWindowProperty(self.window_name, cv2.WND_PROP_VISIBLE) < 1:
                    print("👋 창 닫기(X) 이벤트가 감지되어 플레이어를 종료합니다.")
                    break
            except Exception:
                break
            ...
            # ESC, Q, 또는 Ctrl+C 누름 시 즉시 종료
            if key in [ord('q'), ord('Q'), 27]:
                print("👋 Q / ESC 키 입력으로 플레이어를 즉시 종료합니다.")
                break
            elif key == ord(' '):
        ...
        # 완벽한 리소스 해제 및 프로세스 깔끔 종료
        try:
            cv2.destroyAllWindows()
        except Exception:
            pass
        print("✅ 플레이어가 완전히 종료되었습니다.")
...
if __name__ == "__main__":
    ...
    try:
        player = YouTubeVRPlayer(out_w=args.width, out_h=args.height)
        player.run()
    except KeyboardInterrupt:
        print("\n👋 키보드 인터럽트로 플레이어를 종료합니다.")
    sys.exit(0)
